...
function AppShell({ session }: { session: Session }) {
        ...
        {(!collapsed || mobile) && (
          <div className="flex items-center gap-2.5 px-5 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              <span className="text-sm font-semibold text-white leading-none">Archivio</span>
              <p className="text-[10px] text-slate-400 mt-0.5 leading-none">Secure Document Intelligence for Government</p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-0.5">
          {navItems.slice(0, 4).map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-4 h-4 shrink-0 ${active ? "text-goc-blue-on-navy" : "text-slate-500"}`} />
        ...
        <div className="flex flex-col flex-1 min-h-0 mt-2">
          ...
          <div className="flex items-center justify-between px-3 mb-1.5 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1 text-[11px] font-medium text-goc-blue-on-navy hover:text-white transition-colors"
              title="New chat"
            >
            ...
            {chatSidebar.loadingConversations ? (
              ...
              chatSidebar.conversations.map((conv) => {
                ...
                return (
                  ...
                  >
                    <MessageCircle className={`w-3 h-3 mt-0.5 shrink-0 ${isActive ? "text-goc-blue-on-navy" : "text-slate-600"}`} />
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs truncate leading-snug ${isActive ? "text-white" : "text-slate-400"}`}>
                        {conv.title}
                      </p>
        ...
        {collapsed && !mobile ? (
          <div className="flex flex-col items-center gap-2 py-1">
            <div className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center">
              <span className="text-[11px] font-semibold text-goc-blue-on-navy uppercase">
                {userEmail.charAt(0)}
              </span>
            </div>
  ...
  return (
    <div className="min-h-screen bg-goc-grey">
      {/* Mobile top bar */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-30 flex items-center justify-between bg-goc-navy border-b border-white/10 px-4 h-14">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg bg-white/10 flex items-center justify-center">
            <FileSearch className="w-3.5 h-3.5 text-white" />
          </div>
          <span className="text-sm font-semibold text-white">Archivio</span>
        </div>
        <button
      ...
      {/* Desktop sidebar */}
      <aside
        className={`hidden lg:flex fixed top-0 left-0 bottom-0 z-20 flex-col bg-goc-navy border-r border-white/10 transition-all duration-200 ${
          collapsed ? "w-14" : "w-60"
        }`}
      >
      ...
      <aside
        className={`lg:hidden fixed top-0 left-0 bottom-0 z-20 w-60 flex flex-col bg-goc-navy border-r border-white/10 transition-transform duration-200 ${
...
export default function App() {
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-goc-grey flex items-center justify-center">
